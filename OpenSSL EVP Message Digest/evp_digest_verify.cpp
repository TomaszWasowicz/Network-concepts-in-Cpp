...
int main(int argc, char *argv[])
{
	...
	// Step 1: Create a Message Digest context
	mdctx = EVP_MD_CTX_new();

	// Step 2: Initialise the context
	EVP_DigestInit_ex(mdctx, md, NULL);

	while ((inByte = BIO_read(bio_in, plaintext, EVP_MAX_MD_SIZE)) > 0) {
		// Step 3: Digest (Hashes) data into context
		EVP_DigestUpdate(mdctx, plaintext, inByte);
	}

	// Step 4: Finalize Retrieves result from ctx
	EVP_DigestFinal_ex(mdctx, md_value, &md_len);

	// Step 5: Cleans up digest context ctx
